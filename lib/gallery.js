...
const getMeta = () => {
  return YAML.parse(fs.readFileSync(path.join(process.cwd(), "meta.yaml"), "utf8"));
};
...
export function getGalleryData({ tag = null, artist = null } = {}) {
  ...
  return Object.entries(meta.gallery)
    ...
    .map(([path, info]) => ({
      ...info,
      path: `/images/${path}`,
      tags: info.tags.map((t) => ({ id: t, translation: meta.tags[t].display })),
    }));
}
...
export function getTags() {
  return Object.entries(getMeta().tags).map(([tag, translations]) => ({
    id: tag,
    translation: translations.display,
  }));
}
